...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Wayfcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Wayfcoind.pid)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 10258)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeWayfcoinamount=<n> " + _("Keep N Wayfcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Wayfcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Wayfcoin is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Wayfcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Wayfcoin to complete") << "\n";
    ...
    nAnonymizeWayfcoinAmount = GetArg("-anonymizeWayfcoinamount", 0);
    if(nAnonymizeWayfcoinAmount > 999999) nAnonymizeWayfcoinAmount = 999999;
    if(nAnonymizeWayfcoinAmount < 2) nAnonymizeWayfcoinAmount = 2;
    ...
    LogPrintf("Anonymize Wayfcoin Amount %d\n", nAnonymizeWayfcoinAmount);
