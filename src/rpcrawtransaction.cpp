...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CWayfcoinAddress(addr).ToString());
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    set<CWayfcoinAddress> setAddress;
    if (params.size() > 2)
    {
        Array inputs = params[2].get_array();
        BOOST_FOREACH(Value& input, inputs)
        {
            CWayfcoinAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Wayfcoin address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address))
        {
            entry.push_back(Pair("address", CWayfcoinAddress(address).ToString()));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        CWayfcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Wayfcoin address: ")+s.name_);
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CWayfcoinAddress(script.GetID()).ToString()));
    return r;
}
...
Value searchrawtransactions(const Array &params, bool fHelp)
{
    ...
    CWayfcoinAddress address(params[0].get_str());
    if (!address.IsValid())
