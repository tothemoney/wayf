...
int nAnonymizeWayfcoinAmount = 1000;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Wayfcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Wayfcoin.conf";
               FILE* ConfFile = fopen(ConfPath.string().c_str(), "w");
               fprintf(ConfFile, "listen=1\n");
               fprintf(ConfFile, "server=1\n");
               fprintf(ConfFile, "rpcuser=wayfcoin\n");
               ...
               fprintf(ConfFile, "rpcport=10258\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Wayfcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Wayfcoin will not work properly.");
